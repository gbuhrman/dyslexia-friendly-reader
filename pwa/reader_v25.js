/* reader.js — hardened build (Aug 25, 2025)
   - Red Letter MC applied BEFORE wrapWords()
   - Fast markers pass (string replace)
   - Safe unwrap (preserve children)
*/
(() => {
  'use strict';

  // ---------- Utils ----------
  const $ = (id) => document.getElementById(id);
  const escapeHtml = (s) => s.replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const storyEl = $('story');

  // ===== EPUB → Plain Text extraction (unchanged) =====
  (function (){
    const STORAGE_KEY = 'reader:import';
    const channelName = 'reader';

    async function consumeImport(payload) {
      try {
        const deliver = async () => {
          if (window.reader && typeof window.reader.loadText === 'function') {
            await window.reader.loadText(payload.text, payload.name, payload.meta || {});
            return true;
          } else if (window.reader && typeof window.reader.openFile === 'function') {
            const file = new File([payload.text], payload.name || 'import.txt', { type: 'text/plain' });
            await window.reader.openFile(file, payload.meta || {});
            return true;
          }
          return false;
        };
        let ok = await deliver();
        if (!ok) {
          if (document.readyState === 'loading') {
            await new Promise(res => document.addEventListener('DOMContentLoaded', res, { once: true }));
          }
          ok = await deliver();
        }
        if (ok) {
          try { localStorage.removeItem(STORAGE_KEY); } catch {}
          if (location.hash === '#import') history.replaceState(null, '', location.pathname + location.search);
        } else {
          alert('Reader integration missing: implement reader.loadText(text, name, meta) or reader.openFile(file).');
        }
      } catch (e) {
        console.error('Import error:', e);
      }
    }

    async function extractEpubToPlainText(file) {
      if (!(window.JSZip)) throw new Error('JSZip not loaded (vendor/jszip/jszip.min.js).');
      const zip = await JSZip.loadAsync(file);

      const CONTAINER = 'META-INF/container.xml';
      const containerEntry = zip.file(CONTAINER);
      if (!containerEntry) throw new Error(`EPUB missing ${CONTAINER}`);
      const containerXml = await containerEntry.async('string');
      const containerDoc = new DOMParser().parseFromString(containerXml, 'application/xml');
      const rootfileEl = containerDoc.querySelector('rootfile');
      const opfPath = rootfileEl && rootfileEl.getAttribute('full-path');
      if (!opfPath) throw new Error('Could not locate .opf (no rootfile full-path)');

      const opfEntry = zip.file(opfPath);
      if (!opfEntry) throw new Error(`Missing OPF at ${opfPath}`);
      const opfXml = await opfEntry.async('string');
      const opfDoc = new DOMParser().parseFromString(opfXml, 'application/xml');

      const manifest = {};
      opfDoc.querySelectorAll('manifest > item').forEach(it => { manifest[it.getAttribute('id')] = it.getAttribute('href'); });
      const spineIds = Array.from(opfDoc.querySelectorAll('spine > itemref')).map(el => el.getAttribute('idref')).filter(Boolean);
      const baseDir = opfPath.includes('/') ? opfPath.slice(0, opfPath.lastIndexOf('/') + 1) : '';

      function xhtmlToText(xhtml) {
        const doc = new DOMParser().parseFromString(xhtml, 'text/html');
        doc.querySelectorAll('br').forEach(br => br.replaceWith(doc.createTextNode('\n')));
        doc.querySelectorAll('h1,h2,h3,h4,h5,h6').forEach(h => {
          const t = (h.textContent || '').trim();
          h.replaceWith(doc.createTextNode('\n\n' + t.toUpperCase() + '\n\n'));
        });
        doc.querySelectorAll('p,blockquote,li,pre,section,article').forEach(el => {
          const t = (el.textContent || '').replace(/\s+\n/g, '\n').trim();
          el.replaceWith(doc.createTextNode(t ? t + '\n\n' : ''));
        });
        let out = (doc.body.textContent || '').replace(/\r/g, '');
        out = out.replace(/\u00A0/g, ' ').replace(/\n{3,}/g, '\n\n').trim() + '\n\n';
        return out;
      }

      const chunks = [];
      for (const id of spineIds) {
        const href = manifest[id]; if (!href) continue;
        const entry = zip.file(baseDir + href); if (!entry) continue;
        const xhtml = await entry.async('string');
        chunks.push(xhtmlToText(xhtml));
      }
      return chunks.join('');
    }

    async function openEpubAsPlainText(file) {
      const text = await extractEpubToPlainText(file);
      const filename = (file && file.name) ? file.name.replace(/\.epub$/i, '.txt') : 'book.txt';
      return window.reader.loadText(text, filename, { source: 'epub-extracted' });
    }
    window.openEpubAsPlainText = openEpubAsPlainText;

    if (location.hash === '#import') {
      try { const raw = localStorage.getItem(STORAGE_KEY);
        if (raw) { const p = JSON.parse(raw); console.debug('[Reader] Import payload received', {name:p?.name, id:p?.id}); consumeImport(p); }
      } catch (e) { console.error('Import error:', e); }
    }
    try {
      const bc = new BroadcastChannel(channelName);
      bc.onmessage = (ev) => {
        if (ev?.data?.type === 'ping') { try { ev.target.postMessage({ type:'pong', from:'reader' }); } catch {} return; }
        if (ev?.data?.type === 'import' && ev.data.payload) consumeImport(ev.data.payload);
      };
    } catch {}
  })();

  // ---------- Appearance / prefs ----------
  const autoformatToggle = $('autoformatToggle');
  const fontSelect = $('fontSelect');
  const fontSize = $('fontSize');
  const lineSpacing = $('lineSpacing');
  const textColor = $('textColor');
  const bgColor = $('bgColor');
  const PREFS_KEY = 'df_reader_prefs_v1';

  function loadPrefs(){ try { return JSON.parse(localStorage.getItem(PREFS_KEY)||'{}'); } catch { return {}; } }
  function savePrefs(next){ const cur = loadPrefs(); const out = Object.assign({}, cur, next||{}); try { localStorage.setItem(PREFS_KEY, JSON.stringify(out)); } catch {} return out; }
  function applyPrefs(p) {
    const prefs = p || loadPrefs();
    const target = storyEl || document.body;
    if (!prefs) return;
    if (prefs.fontFamily) target.style.fontFamily = prefs.fontFamily;
    if (prefs.fontSize) target.style.fontSize = prefs.fontSize + 'px';
    if (prefs.lineHeight) target.style.lineHeight = String(prefs.lineHeight);
    if (prefs.textColor) target.style.color = prefs.textColor;
    if (prefs.bgColor) document.body.style.backgroundColor = prefs.bgColor;
    if (fontSelect && prefs.fontFamily) fontSelect.value = prefs.fontFamily;
    if (fontSize && prefs.fontSize) fontSize.value = prefs.fontSize;
    if (lineSpacing && prefs.lineHeight) lineSpacing.value = prefs.lineHeight;
    if (textColor && prefs.textColor) textColor.value = prefs.textColor;
    if (bgColor && prefs.bgColor) bgColor.value = prefs.bgColor;
  }

  // ---------- Formatting / chapters ----------
  function autoFormatText(txt) {
    const paras = txt.split(/\r?\n\s*\r?\n/g).map(s => s.trim()).filter(Boolean);
    const html = paras.map(p => `<p>${escapeHtml(p)}</p>`).join("\n");
    return html || `<p>${escapeHtml(txt)}</p>`;
  }
  function promoteChapterHeadings(){
    if (!storyEl) return;
    const paras = Array.from(storyEl.querySelectorAll('p'));
    let n = 0;
    paras.forEach(p => {
      const t = (p.textContent || '').trim();
      if (/^(chapter|chap\.)\s+([ivxlcdm]+|\d+|[a-z-]+)\b/i.test(t)) {
        const h = document.createElement('h2');
        h.textContent = t;
        if (!h.id) h.id = `ch-${++n}`;
        p.replaceWith(h);
      }
    });
  }
  function detectChapters(){
    const chapterSelect = $('chapterSelect');
    if (!storyEl || !chapterSelect) return;
    const hs = storyEl.querySelectorAll('h1,h2,h3');
    chapterSelect.innerHTML = '';
    if (!hs.length) {
      chapterSelect.innerHTML = '<option value="">(No chapters)</option>';
      return;
    }
    hs.forEach((h, i) => {
      if (!h.id) h.id = `ch-${i+1}`;
      const opt = document.createElement('option');
      opt.value = h.id;
      opt.textContent = h.textContent.trim() || `Chapter ${i+1}`;
      chapterSelect.appendChild(opt);
    });
  }

  // ---------- Word wrapping / TTS ----------
  let words = [];
  function wrapWords() {
    const walker = document.createTreeWalker(storyEl, NodeFilter.SHOW_TEXT, null);
    const toWrap = [];
    while (walker.nextNode()) {
      const t = walker.currentNode;
      if (!t.nodeValue || !t.nodeValue.trim()) continue;
      // In hardened v25, wrap words everywhere, even inside mc-red. This keeps
      // the global words array aligned with the token list so that click-to-read
      // starts reading at the correct index. Colour inheritance ensures
      // wrapped words inside .mc-red still appear red.
      toWrap.push(t);
    }
    toWrap.forEach(t => {
      const parts = t.nodeValue.split(/(\s+)/);
      const frag = document.createDocumentFragment();
      parts.forEach(part => {
        if (/^\s+$/.test(part)) {
          frag.appendChild(document.createTextNode(part));
        } else {
          const span = document.createElement('span');
          span.className = 'word';
          span.textContent = part;
          frag.appendChild(span);
        }
      });
      t.parentNode.replaceChild(frag, t);
    });
    // Recompute the words array to include all span.word elements in DOM order.
    words = Array.from(storyEl.querySelectorAll('span.word'));
  }

  /**
   * Unwrap existing span.word wrappers while preserving the inner text nodes.
   * This is used when toggling Red Letter on or when reapplying markers on load.
   * Without unwrapping first, marker tokens (e.g. ### MCD START) get split across
   * multiple <span> elements and cannot be replaced cleanly.
   * @param {HTMLElement} rootEl
   */
  function unwrapWordSpans(rootEl) {
    if (!rootEl) return;
    const spans = Array.from(rootEl.querySelectorAll('span.word'));
    spans.forEach(sp => {
      const parent = sp.parentNode;
      if (!parent) return;
      while (sp.firstChild) parent.insertBefore(sp.firstChild, sp);
      parent.removeChild(sp);
    });
  }

  /**
   * Apply an inline colour on .mc-red spans to ensure the red text displays
   * correctly on Safari/iOS browsers. Safari can ignore inherited colors when
   * content is dynamically inserted, so we explicitly set both color and
   * -webkit-text-fill-color.
   * @param {HTMLElement} rootEl
   */
  function enforceMcColor(rootEl) {
    if (!rootEl) return;
    const nodes = rootEl.querySelectorAll('span.mc-red');
    nodes.forEach(n => {
      n.style.color = 'darkred';
      n.style.webkitTextFillColor = 'darkred';
      n.style.webkitTextStroke = '0px transparent';
    });
  }

  // ---------- Red Letter MC (markers-first, fast) ----------
  function hasMCDMarkers(rootEl) {
    if (!rootEl) return false;
    const txt = rootEl.textContent || "";
    return /###\s*MCD\s*START/i.test(txt) || /###\s*MCD\s*END/i.test(txt);
  }

  // Fast string-based markers pass: replace the literal tokens with span tags.
  // Safe here because our source is plain text that we wrapped into <p> with escaped content.
  function applyMarkersFast(rootEl) {
    if (!rootEl) return;
    const html = rootEl.innerHTML;
    const next = html
      .replace(/###\s*MCD\s*START/gi, '<span class="mc-red">')
      .replace(/###\s*MCD\s*END/gi, '</span>');
    if (next !== html) rootEl.innerHTML = next;
  }

  function unwrapRedPreserveChildren(rootEl){
    if (!rootEl) return;
    const spans = Array.from(rootEl.querySelectorAll('span.mc-red'));
    spans.forEach(sp => {
      const parent = sp.parentNode;
      if (!parent) return;
      // Move children out, then remove wrapper
      while (sp.firstChild) parent.insertBefore(sp.firstChild, sp);
      parent.removeChild(sp);
    });
  }

  // We keep the heuristic machinery off for now (your priority is markers-only).
  // Hook is left here for future re-enable.
  function applyRedLetterIfEnabledNow() {
    const root = $('story');
    if (!root) return;
    if (window.__mc_enabled !== true) return;

    if (hasMCDMarkers(root)) {
      // Apply markers to raw HTML, THEN re-wrap words for TTS/click-to-read
      applyMarkersFast(root);
      wrapWords();
    } else {
      // Heuristic path (disabled for stability). No-op:
      console.info('[Reader] No MCD markers found — heuristic red-lettering is currently disabled in hardened build.');
    }

    promoteChapterHeadings();
    detectChapters();
  }

  // ---------- Reader API ----------
  const fileInput = $('fileInput');
  const clickReadToggle = $('clickReadToggle');
  const speedControl = $('speedControl');
  const voiceSelect = $('voiceSelect');
  const readAloudBtn = $('readAloudBtn');
  const stopReadAloudBtn = $('stopReadAloudBtn');
  const chapterSelect = $('chapterSelect');
  const bookmarkBtn = $('bookmarkBtn');
  const gotoBookmarkBtn = $('gotoBookmarkBtn');
  const toggleReadingBtn = $('toggleReadingBtn');
  const quickControlsBtn = $('quickControlsBtn');

  // Tokens for TTS
  function getAllTokens() {
    const raw = (storyEl && (storyEl.innerText || storyEl.textContent || "")) || "";
    const flat = raw.replace(/\s+/g, " ").trim();
    return flat ? flat.split(" ") : [];
  }

  // TTS
  let voices = [], utter = null, clickReadMode = false, currentSpokenIndex = -1;
  function loadVoices() {
    voices = (typeof speechSynthesis !== 'undefined') ? speechSynthesis.getVoices() || [] : [];
    voiceSelect.innerHTML = "";
    const def = document.createElement('option');
    def.value = "";
    def.textContent = voices.length ? "Default" : "No voices available";
    voiceSelect.appendChild(def);
    voices.forEach(v => {
      const o = document.createElement('option');
      o.value = v.name;
      o.textContent = `${v.name} — ${v.lang}${v.default ? " (default)" : ""}`;
      voiceSelect.appendChild(o);
    });
  }
  function getSelectedVoice() {
    const name = voiceSelect.value;
    if (!name) return null;
    return voices.find(v => v.name === name) || null;
  }
  function speakFrom(startIndex = 0) {
    if (!("speechSynthesis" in window)) return;
    const tokens = getAllTokens();
    if (!tokens.length) return;

    const CHUNK = 40;
    let pos = Math.max(0, Math.min(tokens.length - 1, startIndex || 0));
    const v = getSelectedVoice();

    try { speechSynthesis.cancel(); } catch {}

    function next() {
      if (pos >= tokens.length) return;
      const end = Math.min(tokens.length, pos + CHUNK);
      const text = tokens.slice(pos, end).join(" ");
      const u = new SpeechSynthesisUtterance(text);
      if (v) u.voice = v;
      const rate = parseFloat((speedControl && speedControl.value) || "1");
      u.rate = (rate > 0 ? rate : 1);
      u.onend = () => { pos = end; try { localStorage.setItem('df_reader_bookmark', JSON.stringify({ tok: pos, scroll: window.scrollY || 0 })); } catch {} setTimeout(next, 20); };
      try { speechSynthesis.speak(u); } catch {}
    }
    setTimeout(next, 50);
  }
  function stopSpeaking() { try { speechSynthesis.cancel(); } catch {} utter = null; }

  // Public reader API
  window.reader = window.reader || {};
  window.reader.loadText = async function loadText(text, name, meta = {}) {
    if (!storyEl) return;

    // 1) Render text
    const useAuto = (autoformatToggle && autoformatToggle.checked) !== false;
    storyEl.innerHTML = useAuto ? autoFormatText(text) : `<pre>${escapeHtml(text || '')}</pre>`;

    // 2) Apply prefs
    applyPrefs();

    // 3) If MC toggled on, ensure markers are applied before wrapping words
    if (window.__mc_enabled === true && hasMCDMarkers(storyEl)) {
      // Unwrap existing .word spans so marker tokens are contiguous
      unwrapWordSpans(storyEl);
      applyMarkersFast(storyEl);
    }

    // 4) Word-wrap AFTER red-letter so we don't churn twice
    wrapWords();
    // 4b) Ensure red text colour is applied for Safari/iOS
    enforceMcColor(storyEl);

    // 5) Chapters
    promoteChapterHeadings();
    detectChapters();

    if (meta && meta.title) { try { document.title = `${meta.title} — Dyslexia-Friendly Reader`; } catch {} }
  };

  window.reader.openFile = async function openFile(file, meta = {}) {
    const isEpub = /\.epub$/i.test(file.name || '') || file.type === 'application/epub+zip';
    if (isEpub) return window.openEpubAsPlainText(file);
    const text = await file.text();
    return window.reader.loadText(text, file.name || 'import.txt', meta);
  };

  // ---------- Events ----------
  function setControlsOffset() {
    const controls = $('controls');
    const root = document.documentElement;
    const offset = document.body.classList.contains('reading') ? 0 : (controls ? controls.offsetHeight : 0);
    root.style.setProperty('--controls-offset', offset + 'px');
  }
  function enterReadingMode() {
    document.body.classList.add('reading');
    if (quickControlsBtn) quickControlsBtn.hidden = false;
    setControlsOffset();
    setTimeout(() => window.scrollTo(0, 0), 0);
  }
  function exitReadingMode() {
    document.body.classList.remove('reading');
    if (quickControlsBtn) quickControlsBtn.hidden = true;
    setControlsOffset();
  }

  document.addEventListener('DOMContentLoaded', async () => {
    applyPrefs();

    // Voices
    loadVoices();
    if (typeof speechSynthesis !== 'undefined' && speechSynthesis.onvoiceschanged !== undefined) {
      speechSynthesis.onvoiceschanged = loadVoices;
    }

    // Pref controls
    const target = storyEl || document.body;
    if (fontSelect) fontSelect.addEventListener('change', () => { target.style.fontFamily = fontSelect.value || ''; applyPrefs(savePrefs({fontFamily: fontSelect.value})); });
    if (fontSize)  fontSize.addEventListener('input', () => { const v = parseInt(fontSize.value||'18', 10); target.style.fontSize = v + 'px'; applyPrefs(savePrefs({fontSize: v})); });
    if (lineSpacing) lineSpacing.addEventListener('input', () => { const v = parseFloat(lineSpacing.value||'1.6'); target.style.lineHeight = String(v); applyPrefs(savePrefs({lineHeight: v})); });
    if (textColor) textColor.addEventListener('input', () => { const v = textColor.value || '#000000'; target.style.color = v; applyPrefs(savePrefs({textColor: v})); });
    if (bgColor)    bgColor.addEventListener('input', () => { const v = bgColor.value || '#f8f8f8'; document.body.style.backgroundColor = v; applyPrefs(savePrefs({bgColor: v})); });

    if (speedControl) speedControl.addEventListener('input', () => { if (utter) { try { speechSynthesis.cancel(); } catch {} speakFrom(0); } });
    if (readAloudBtn) readAloudBtn.addEventListener('click', () => { if (!words.length) wrapWords(); speakFrom(0); });
    if (stopReadAloudBtn) stopReadAloudBtn.addEventListener('click', stopSpeaking);
    if (clickReadToggle) clickReadToggle.addEventListener('change', () => { clickReadMode = clickReadToggle.checked; });

    if (storyEl) storyEl.addEventListener('click', (e) => {
      if (!clickReadMode) return;
      const w = e.target && e.target.closest('.word'); if (!w) return;
      if (!words.length) wrapWords();
      const idx = words.indexOf(w);
      if (idx>=0) speakFrom(idx);
    });

    if (fileInput) fileInput.addEventListener('change', async (e) => {
      const f = e.target.files && e.target.files[0];
      if (!f) return;
      const isEpub = /\.epub$/i.test(f.name) || f.type === 'application/epub+zip';
      if (isEpub) { await window.openEpubAsPlainText(f); return; }
      const txt = await f.text();
      await window.reader.loadText(txt, f.name || 'import.txt', {});
    });

    if (chapterSelect) chapterSelect.addEventListener('change', () => {
      const id = chapterSelect.value; if (!id) return;
      const el = document.getElementById(id);
      if (el) el.scrollIntoView({behavior:'smooth', block:'start'});
    });

    if (toggleReadingBtn) toggleReadingBtn.addEventListener('click', () => {
      const on = !document.body.classList.contains('reading');
      toggleReadingBtn.setAttribute('aria-pressed', String(on));
      on ? enterReadingMode() : exitReadingMode();
    });
    if (quickControlsBtn) quickControlsBtn.addEventListener('click', () => { exitReadingMode(); });

    if ($('bookmarkBtn')) $('bookmarkBtn').addEventListener('click', () => {
      const tok = 0; try { localStorage.setItem('df_reader_bookmark', JSON.stringify({ tok, scroll: window.scrollY || 0 })); } catch {}
    });
    if ($('gotoBookmarkBtn')) $('gotoBookmarkBtn').addEventListener('click', () => {
      try { const bm = JSON.parse(localStorage.getItem('df_reader_bookmark')||'null'); if (bm && typeof bm.scroll === 'number') window.scrollTo({ top: bm.scroll, behavior: 'smooth' }); } catch {}
    });

    // Red Letter Toggle (markers-first)
    const mcBtn = $('toggle-mc');
    if (mcBtn) {
      window.__mc_enabled = false;
      const renderMcBtn = () => {
        mcBtn.setAttribute('aria-pressed', String(window.__mc_enabled === true));
        mcBtn.classList.toggle('active', window.__mc_enabled === true);
        mcBtn.textContent = (window.__mc_enabled === true) ? 'Red Letter MC (on)' : 'Red Letter MC (off)';
        mcBtn.title = (window.__mc_enabled === true) ? 'Turn off Red Letter MC' : 'Turn on Red Letter MC';
      };
      renderMcBtn();

      mcBtn.addEventListener('click', () => {
        const nowOn = !(window.__mc_enabled === true);
        window.__mc_enabled = nowOn;
        if (nowOn) {
          // When turning on: unwrap existing .word spans so marker tokens are contiguous,
          // apply markers if present, then re-wrap words and enforce colour
          unwrapWordSpans(storyEl);
          if (hasMCDMarkers(storyEl)) {
            applyMarkersFast(storyEl);
          }
          wrapWords();
          enforceMcColor(storyEl);
        } else {
          // When turning off: unwrap .mc-red spans, then re-wrap words
          unwrapRedPreserveChildren(storyEl);
          wrapWords();
        }
        // Refresh chapters after DOM surgery
        promoteChapterHeadings();
        detectChapters();
        renderMcBtn();
      });
    }

    setControlsOffset();
    window.addEventListener('resize', setControlsOffset);
    window.addEventListener('orientationchange', setControlsOffset);
    window.addEventListener('scroll', () => {
      if (!document.body.classList.contains('reading')) {
        // simple auto-hide
        const y = window.scrollY || 0; document.body.classList.toggle('hide-controls', y > 12);
      }
    });

    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('service-worker.js').catch(() => {});
    }
  });
})();
