...
function detectChapters(){
 ...
 function speakFrom(startIndex = 0) {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Library
    try {
      const res = await fetch("https://gbuhrman.github.io/dyslexia-friendly-reader/pwa/catalog.json", {cache:'no-cache'});
       if (!res.ok) throw new Error(`Failed to fetch ${url}`);
    ...
    try {
      const cat = await loadCatalogJSON("https://gbuhrman.github.io/dyslexia-friendly-reader/pwa/catalog.json");
